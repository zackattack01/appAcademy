...
## original game loop logic

	# def begin_original_game_loop
	# 	a=[*1..n].reverse
	# 	b=[]
	# 	c=[]
	# 	rods = [a, b, c]
	# 	rod_hash = { 'a' => 0, 'b'=> 1, 'c' => 2 }
	# 	rods_inverted = rod_hash.invert
	# 	rods.each{ |rod| p rod }
	# 	while [b, c].any?{|rod| rod != [*1..n].reverse }
	# 		puts "Pick a peg from rods a, b, or c"
	# 		from=rod_hash[gets.chomp.to_s]
	# 		puts "Invalid selection" or next if from.nil?
	# 		if rods[from].empty? 
	# 	     puts "Try again, #{rods_inverted[from]} is empty"
	# 		   rods.each{ |rod| p rod }
	# 		   next
	# 		end
	# 		puts "Place the peg on..."
	# 		to=rod_hash[gets.chomp.to_s]
	# 		puts "Invalid selection" or next if to.nil?
	# 	    if (rods[to]!=[] && rods[from].last>rods[to].last)
	# 	       puts "Try again, you cannot place a larger disc on top of a smaller one"
	# 		   rods.each{ |rod| p rod }
	# 		   next
	# 		end
